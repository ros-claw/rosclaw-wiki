...
Unlike methods that break down a complex task into a sequence of subgoals or verbalized steps, direct action prediction uses a single end-to-end mapping from sensor data (e.g., images, LiDAR) to motor commands. This approach is often implemented using large language models (LLMs) fine-tuned or prompted for action sequences, but it omits any explicit reasoning chain. It serves as a practical baseline to evaluate the value of structured reasoning in navigation and manipulation tasks.
